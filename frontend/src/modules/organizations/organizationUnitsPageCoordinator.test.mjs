...
import { createOrganizationUnitsPageCoordinator } from './organizationUnitsPageCoordinator.ts';
import { loadOrganizationUnits } from './organizationUnitsState.ts';
...
function harness(loadState) {
  ...
  const coordinator = createOrganizationUnitsPageCoordinator({
...
test('new generations suppress stale success and stale error results', async () => {
  for (const staleState of [
    ...
    await staleLoad;
    current.resolve({ status: 'success', units: [] });
    await currentLoad;
...
test('active 401 clears the token and navigates to login', async () => {
  let clearCount = 0;
  const setup = harness((isRequestActive) => loadOrganizationUnits({
    readUnits: async () => { throw new ApiError('Unauthorized', 'http', 401); },
...
test('stale 401 cannot clear the token, navigate, or replace the current result', async () => {
  ...
  let clearCount = 0;
  const setup = harness((isRequestActive) => loadOrganizationUnits({
    readUnits: () => requests.shift().promise,
    clearSession: () => { clearCount += 1; },
  }, isRequestActive));
